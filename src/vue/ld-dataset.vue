<template>
  <ul class="ld-taxonomy">
    <li v-for="(row, index) in rows" :key="index" :class="{ expanded: expanded[row.uri.value], 'is-subject': subject === row.uri.value }">
      ...
      <ld-dataset v-if="expanded[row.uri.value]" :endpoint="endpoint" :graph="graph" :subject="subject" :predicate="predicate" :parent="row.uri.value" :chain="conceptChain"></ld-dataset>
    </li>
  </ul>
</template>
    ...
    fetchChain() {
      ...
      this.runQuery(query, (response) => {
        ...
        this.conceptChain = rows.filter((row) => {
          return (row.type.value === 'http://www.w3.org/ns/dcat#Dataset');
        }).map((row) => {
        ...
        let schemeRows = rows.filter((row) => {
          return (row.type.value === 'http://www.w3.org/ns/dcat#Catalog');
        });
